using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Euler8
{
    class Program
    {
        static void Main(string[] args)
        {
            string strNum= "";
            string numm = "";
            long max = 0;

            for (int i = 0; i <= 995; i++)
            {
                string sub = strNum.Substring(i, 5);
                if (!sub.Contains("0"))
                {
                    long product = getDigitProduct(sub);

                    Console.WriteLine(sub + " = " + product);

                    if (product > max)
                    {
                        max = product;
                    }
                }
            }
               Console.WriteLine(max);
        }

        public static long getDigitProduct(string strNum)
        {
            int intVal = Convert.ToInt32(strNum);
            int org = intVal;
            long product = 1;
            List<int> digits = new List<int>();

            int i = 0;

            while(intVal >= 1)
            {
                digits.Add(intVal % 10);
                intVal = intVal / 10;
            }
            
            Console.WriteLine();

            foreach (var dig in digits)
            {
                Console.Write(dig+",");
                product *= dig;
            }
            Console.WriteLine();
            return product;
        }
    }
}
